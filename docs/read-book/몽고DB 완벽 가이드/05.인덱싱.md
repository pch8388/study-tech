...
# expalin 출력
expalin은 쿼리에 대한 많은 정보를 제공하고, 느린 쿼리를 위한 중요한 진단 도구
...
- `_id`는 자동으로 유니크하게 생성됨
  - 삭제 불가능 고유 인덱스
> 고유 인덱스가 걸려있는 데, 아예 키를 입력하지 않으면 null로 삽입이 된다. null인 도큐먼트가 이미 있다면, 두번째부터는 실패한다.
...
## 부분 인덱스
- 오직 키가 존재할 때만 고유 인덱스를 존재하게 하고 싶을 때. 죽, 고유한 필드가 존재하거나 아예 존재하지 않으면 `unique`와 `partial`을 결합할 수 있다
